...
右值引用时c++0x引入的一个核心概念，在这段学习时间里对它的认知是个渐进的过程，每每认为自己理解它了，每每过段时间否定自己
的认知。到现在，我还是无法用准确的语言来描述它，这里我使用FAQ的方式阐述一下我对它的理解。

Q: 什么是右值？

A: 等号左边的值，可能是个变量，也可能是个常理。比如：

	int n = 10;
	int m = n;

等号左边的值一定是个可地址化的东西，我们可以给一个变量赋值但是无法给一个常量赋值。所以左边叫左值，右边叫右值。
...
A: 到底是变量还是常量，编译器可以跟踪到。这不用担心，就算你对一个常理引用取地址，它也可以在堆栈里造出个变量让你取地址。
简单的说，对于编译器来说，每次赋值都是不同的变量。

Q: 如果是常量，我们已经有了const了，而且工作的很好。

A: 确实在类成员变量定义和函数体变量定义里基本不会看到右值引用，除非你非要这么写。而且也确实没有必须这么写的必要。

Q: 那它经常在什么场合出现？

A: 在函数参数里出现，这也是它唯一必须出现的地方。
...
A: 这是跟copy语意对应的一个语意。c++有copy构造和copy赋值语意。比如说std::string，如果我们生成一个临时的std::string，这很常见
在string加(cat)的时候就会生成。copy语意会在最终结果上进行一次内存赋值。比如：

	std::string str;
	std::string some = "aaaaaa";
	str = some + "bbbbb";

some加上"bbbbb"后会产生一个新的std::string实例，然后对str进行copy赋值。str会生成一块内存，然后进行strcpy或者memcpy之类的操作。
这显然不理想。因为相加的结果是个临时对象，复制后它还要析构，释放自己的持有的内存。最好的办法是把自己持有的内存给str，这样开销
最小。以前的c++没有move机制，只能copy，这严重影响了效率，特别是stl这类类库。

c++0x标准把move描述成"偷"。str"偷"了some + "bbbbbb"产生临时结果的内存。

Q: 这是个怎么样个过程？c++0x支持"偷"操作么？

A: c++本身不支持"偷"的具体操作，怎么"偷"每种类自己决定。我们假设std::string是这样的：

	struct string {
		char *_ptr;
		string(string &&x) : _ptr() {
			std::swap(_ptr, x._ptr);
		}
	};

当对它赋值的时候，它会申请一块内存并且copy字符串到这个内存。如果要执行一个move语意构造或者复制，新的对象会通过代码窃取对方的_ptr。
严格的说，这不是move这是swap语意。move构造或者赋值去交换对方的_ptr。

为什么是交换？可能str里也有内存，它窃取临时对象的_ptr并且把自己的_ptr交换给对方。对方析构的时候可以释放内存，这样不会导致内存泄漏
或者逻辑丢失。

Q: std::swap是什么？

A: 跟个swap宏差不多，tmp = a, a = b, b = a。a需要具有move或者copy构造还有move或者copy赋值，b需要move或者copy赋值。看前面那段代码
就能推测出。噢，我不承认，我根本没想，编译器会有默认的。

Q: 我们已经有copy构造了，这个"偷"怎么体现？

A: 所以才有个右值引用，c++标准制造出这么个玩意，用来重载函数。例子见上面。

Q: 到底什么时候调用copy语意，什么时候调用move语意？

A: 右值环境下调用move，左值环境下调用copy。比如说上面的例子string cat返回的是string就是右值，返回的是string&就是右值。编译器来判断。

Q: 这岂不是很难判断？

A: 是啊，有点难于判断。不过，你要相信编译器。同时相信自己的代码。你返回的是类实例，它就用move语意，反之就是copy语意。

Q: 我是个传统的程序员，我只声明了copy语意。会是什么情况？

A: 只会调用copy语意。在函数匹配的过程中，会有个降解判断过程。看一下`std::decay`，这就是最标准的函数匹配标准降解过程。

Q: 噢，我对这些语意有点了解了。如果我想强制类的其它实例去"偷"我该怎么做？
...
A: 如果类成员是基础类型，或者叫做标量类型，int啊，指针啊，引用啊，你可以自由操作决定是copy还是swap。否则，尽量去调用它的
move构造或者赋值。也就是使用一次`std::move`。例如。
...
构造和赋值的时候忘记使用它，这会导致你无法达到设计目标。

Q: 尽管你说只是在move语意的构造和赋值上，但是经常在模板库的函数参数上看到很多右值引用`&&`。

A: 这是右值引用的另外一个作用：完美参数传递。
...
对于一个正常的类，就会产生构造，如果是指针哪？更不可知。我们一般要想很多的基于const的重载来完成这个事情。但是，问题是
...
大约是这个样子。这些函数的参数都是引用，只是右值和左值而已。std::forward，在增加个右值上去，继续传递下去。这个函数到底是否是const
由最终函数的实现决定。
...
这种情况下，实例化后的模板已经知道_Args是什么，他只会提出所有的参数表是右值引用。会报错。其实，不是所有的模板函数在变参上都要写
右值的，你可以很自信的去掉它。除非你不太自信，可能还有其它需求。这时候，你可以这么写。
...
总之把，c++是个利器，看你怎么用。我对c++感觉气馁，它的概念令人迷糊。坦率的说我一点都不爱它。
