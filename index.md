...
------------------------------------------------------------------------------------------------------------------------------------------
##2013-10-16 C++0x的右值引用

在c++0x及其以后规范版本中右值引用是个核心概念，在我学习使用c++0x到现在的过程中精确掌握右值引用始终是个主要问题。写着写着程序
就去反复翻看标准文档或者去标准头文件中琢磨人家的实现意图，几乎每天都会发生。我经常感觉我已经接近掌握它了，但是每每过
几天我就否定了自己。

这个blog中阐述的概念是我现在对这东西的理解，肯定有很多地方不对，仅供参考，切勿当作标准。

我感觉，右值引用之所以令人难于理解是因为它涉及了太多的编译优化相关概念，另外传统程序设计的习惯性思维也在阻碍我去理解它。
人们习惯于实实在在的事物，对于虚幻的无法捉摸的东西总是感觉很虚渺。以前C++的引用是个很实在的东西，右值引用则显得飘渺不定。
但是右值引用对于编译器来说是个实实在在的东西。使用编译器的思维，或者猜测编译器如何去实现，对于理解右值引用还有sfinae很有
帮助。sfinae在c++0x中是广泛存在的，就是你从来没写过它们也可能天天在用它们，那些标准头里的enable_if铺天盖地。

在c++0x里右值有3个重要概念：右值引用、move语意和完美模板参数forward。

首先阐述我对右值引用的理解。

很多文章说右值引用时对无法直接取值的变量的引用。这非常正确。但是，我认为也不正确。
...
这个tmp是逻辑概念，但是对于编译器来说确实存在的。在以前的处理中它被‘折叠’成`const foo&`，来完成foo的copy assign。

tmp只是个编译器级的临时变量，但是在以前的c++中它缺少一种语意。

第一个问题，move语意

C++有默认构造函数、copy构造函数，这是大家耳熟能详的了。比如：

	struct foo {
		char *_buff;
		foo() {
			_buff = (char*)malloc(100000);
		};
		foo(const foo &x) {
			_buff = (char*)malloc(100000);
			memcpy(_buff, x.buff, 100000);
		}
		~foo() {
			free(_buf);
		}
...
这时候，调用了foo的copy构造，并进行了大量的内存赋值。或许我们只是希望交换一下_buff，或许我们希望copy整个_buff。
在语意级上，以前的C++缺少了交换的概念。临时变量都会转换成copy构造或者copy赋值。

甚至有时候，我们需要一个唯一持有者。比如说一个函数中的guard，我只希望调用一次。但是copy构造会复制多个，也就
产生多次调用。

这时候，我们需要一个swap语意，进行数据的交换。但是在以前的c++中没有这个语意。

	f = clone(foo());

c++0x中增加了这个语意，告诉编译器这是个move构造，其实swap构造更精确。在c++标准中称之为“偷”，把其它对象偷
过来，被偷对象析构旧的数据。
...
上面的例子，调用move构造偷取了_buff。被偷的就不会去free。这样c++0x通过右值引入构造，产生了偷的构造。
这个偷的构造不是自动产生的，你必须主动要求它去偷。

如何让一个普通的变量被偷？你需要强制转换它为右值。也就是std::move()。我以前一直不清楚这个函数在干什么，我觉得一个函数
就该有点实际的作用。它只是个强制转换，把一个类型转换成右值。ok，这时候类型不是foo&或者const foo&了，这可以满足调用
foo(foo &&x)的overload需要了。

std::move除了强制转换外，没有其它操作。它只是告诉编译器，这是个右值了。如果foo有右值copy或者assign就可以调用。如果没有
根据匹配原则会调用copy 构造或者copy assign。

潜台词是这东西可偷，至于你是否偷，还是怎么偷？看你自己决定。在偷的过程中右值引用有什么不同么？没有，跟普通的引用一样。
可以取子对象可以取地址。

我们几乎很难，甚至不可能在函数体里看到右值引用。就算看到了也基本是这样。

	foo f = std::move(xxoo);

这告诉f，你要执行move构造。其它的右值转换，几乎无编写意义。就是上面的代码，也只是及特殊的例子。

这种代码几乎都出现在copy或assign上。


第二个问题，完美参数传递，std::forward。

在c++的模板编程和使用中，我们会没完没了的调用模板函数。这里面有2个层面的内容。第一在模板推导上，总是被推导成引用。

	template <typename _T>
	void foo(_T t);

t被推导成了引用。在以前的c++引用的用被推导成引用，指针被推导成指针的引用。c++还有两个变量修饰符，const和volatile,
它们都影响函数overload。如果你想写个相当牛X的模板函数，你需要写很多的cv品质overload。cv品质就是const和volatile。

如果，我们的函数只有一个参数，这至少是2个函数，如果更多。。。就非常困难了。

模板编程就像c的宏，在没有跨越调用边界的时候，比如说某个非模板静态函数或者类成员函数。编译器能够跟踪某个变量的类型。
std::forward的作用是保持类型和品质，其本质也是类型转换。

我在早期一直奇怪move和forward到底在做什么。甚至所有的模板变参都是`_Args&&...·。我那时候在想，它们肯定干了我不知道
的操作。其实，没有其它，它们就是类型转换。

网上说的 & &、 & &&、 && &、 && &&，的匹配过程在这时候体现。这看上去有点复杂，但是最终结果就是const还是const，
非const还是非const。程序员只需要forward就行了，最终由可跨越调用边界的具体函数来判断，到底是哪个。

在我写的早期模板里，经常会因为使用逻辑吐出一大堆的错误。在g++里，经常会让满屏都是通红的。这大多数都是最终匹配
不符合导致的结构。我需要逐个递归向上找到我设计的错误。后来，我开始意识到这种排查的复杂度，开始尽量在封装上避免
这种瀑布式的错误输出。

总之，在我看来，右值引用就是个更宽泛的引用。可以引用常理，可以引用变量。关键在于它可以暗示进行偷构造。它的forward
机制和匹配，甚至不需要去理解只要是按部就班去写，就会在模板传递中产生作用。

但是在使用的时候，需要知道类型确认的时机，比如：

	template<typename sig, typename _Functor>
	class fun;
	
	tempalte<typename _R, typename ..._Args, typename _Functor>
	struct fun<_R(_Args), _Functor> {
		_Functor _fn;
		_R operator()(_Args&&...args);
	}

这就是个不恰当的例子，Args的类型已经确定。导致operator()需要所有参数都是右值。作为一个模板类封装者，你需要知道&&的所有
含义。

c++11是个高度理想化的语言。有时候我们会碰到一些问题，这基本都是对标准吃不透。但是这个标准，实在是太复杂了。我今天
就碰到一个问题，好在我比较坚持，解决了。
...
我折腾了2个多小时，_T的推导出类型不对，所有fn都被强制转换成了foo<_A>，累的我够呛。我当时坚信这不是个bug，因为太明显了。
当看到g++的function的move construct参数中的Useless时，我乐了。回头翻看c++标准关于move构造的描述，这个模板确实有move
构造的嫌疑。

c++这东西在语意级的复杂度算是登峰造极了。不谦虚的说，我几乎可以熟练使用所有语言，只需要给我几天时间。但是c++11，我已经
用了2个多月了，我不确切的地方应该还有很多。

在接触这东西的早期，我看了Modern C++ Design，它传递给我一些信息，比如typelist。但是不是很确切，我也确实没有反复读它。
之后，我参考的文档只是http://en.cppreference.com/。真正敲开我的c++11之路的是那些sfinae，member.h中的代码。

这段时间，非功能类的代码都在剧烈的变动。我希望能够找到一个好的模式，我对boost的function和bind不太喜欢，但是对于
那些类型判断比较能够接受。

在写完closure后，我在写异步io的应用中感觉比较舒畅。想传递什么就传递什么。我甚至觉得这东西，不符合我保守的思想。
前天，我写某个函数写得有点烦躁。这个函数的出现频率不高，但是相当的繁琐，很多的条件判断。这让我想起以前的
guard类。ok，我去做一个。libll++中，除了类型判断诊断类，都是自实现的。这不是想否定boost，这是个自我挑战。
在lib++中，内存处理超强的强调分配器，这导致guard去释放个指针是个浮云。所以，这个guard只是个灵活的final调用。

我希望能够让它有较高的效率。以前的closure实现了很多的functor，这次我把他们迁移到functor.h中。并且给他们划分了层次。
make_fn，封装了functor的封皮。make_functor，在make_fn基础上封装了函数签名。closure使用了functor，哎我又重写了一遍
closure。这么做的目的是我希望能够静态构造一个functor，作为guard的实现基础。
make_fn、make_functor和closure的封装，我估计不会再更改了。因为我已经吃透他们了，至少我感觉是这样。
